...
contract Root is Ownable {
    ...
    mapping(Group => GroupInfo) groupToInfo;
    uint256 groupToInfoCount;
    uint256 totalGroupsBalance;
    ...
    /// @notice Check total balance when all group loaded
    function _checkTotalGroupsBalance() private view {
        if (groupToInfoCount >= 9) {
            require(totalGroupsBalance == 127500000000000000000000000, "[E-96] - Invalid total balance.");
        }
    }
    ...
    function _getWithdrawPercent(GroupInfo memory _groupInfo) private view returns(uint256) {
        ...
        uint256 _dayWithdrawPercent = (_groupInfo.percents[_index + 1]).div(30);
        uint256 _daysFromDeploy = (block.timestamp).sub(_deployTime).div(24 * 3600).mod(30);
        uint256 _daysWithdrawPercent = _dayWithdrawPercent.mul(_daysFromDeploy);
